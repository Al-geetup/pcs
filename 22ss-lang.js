...
(function() {
    ...
    // … inside (function(){ but before any DOM creation …
    const translations = {
        en: { /* … */ },
        fr: { /* … */ },
        ar: { /* … */ }
    ...
    // ── after translations & before any DOM work ──
...
// ── Add once, after registrationFormHTML() definition ──
function bindRegistrationHandlers() {
    // “Start chatting” on the welcome screen
    ...
    // Chat controls (only now do these exist)
...
// ───────────────────────────────────────────────
    ...
    `;
            ...
            // 1) show the language picker first
        chatWindow.innerHTML = languagePickerHTML;

        // 2) when they click a lang‑button, swap in the form + chat interface
        chatWindow.querySelectorAll('.lang-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                  currentLang = btn.dataset.lang;
                  chatWindow.innerHTML = registrationFormHTML() + chatInterfaceHTML;
    ...
    async function handleRegistration(event) {
        ...
        // grab fresh references *after* the innerHTML swap:
        const nameInput         = chatWindow.querySelector('#chat-user-name');
        const emailInput        = chatWindow.querySelector('#chat-user-email');
        const phoneInput        = chatWindow.querySelector('#chat-user-phone');
        const nameError         = chatWindow.querySelector('#name-error');
        ...
        if (!phone) { phoneError.textContent = 'Please enter your phone number'; phoneInput.classList.add('error'); isValid = false; }
        if (!isValid) return;
      
        // now your existing loadPreviousSession + fetch code…
    ...
    async function submitMessage(messageText) {
        ...
        try {
          ...
          errMsg.textContent = "Sorry, I couldn't send your message. Please try again.";
